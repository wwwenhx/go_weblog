...
// 根据方法名调用方法
func callMethod(methodName string, args interface{}) (*pb.MsgRes, error) {
	res := &pb.MsgRes{}
	err := errors.New("")
	method := reflect.ValueOf(service.GetMsgSrv()).MethodByName(methodName)
	if method.IsValid() {
	...
	if len(result) > 0 {
		res = result[0].Interface().(*pb.MsgRes)
